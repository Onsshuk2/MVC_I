...
namespace pv311_mvc_project.Controllers
{
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> ApplyPromoCode(string promoCode)
        {
            ...
            decimal isPromoValid = await _cartService.ApplyPromoCodeAsync(promoCode);

            if (isPromoValid==0)
            {
                // Apply the discount and save it to the session or ViewModel
                var promo = await _cartService.GetPromoAsync(promoCode);
                HttpContext.Session.SetString(Settings.SessionPromoKey, promoCode);

                // Update the ViewModel with the applied discount
                var cartItems = _cartService.GetItems();
                var products = _productRepository
                    .GetAll()
                    .Where(p => cartItems.Select(i => i.ProductId).Contains(p.Id))
                ...
                return View("Index", viewModel); // Return the updated view with applied discount
            }
            else
            {
                // Handle invalid promo code (show an error or notify the user)
                TempData["ErrorMessage"] = "Невірний промокод!";
